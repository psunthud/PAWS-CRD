...
Module Storage
    ...
    Public Const Banner As String = "PAWS-CRD" & vbCrLf & "version 1.0 (February 2010)" & vbCrLf & "Developers: Sunthud Pornprasertmanit & W. Joel Schneider" & vbCrLf
    ...
    Public Function FindVar(ByVal nT As Integer, ByVal nC As Integer, ByVal nIndiv As Integer, ByVal residualBetween As Double, ByVal residualWithin As Double, Optional ByVal degreeCertainty As Double = -1) As Double
        ...
        Dim VarGroupMean As Decimal = (residualWithin + (nIndiv * residualBetween)) / nIndiv
        If degreeCertainty <> -1 Then
            Dim standardscore As Decimal = InvNormalDistribution(degreeCertainty)
            Dim df As Decimal = J - 2
            If residualBetween <> 0 Then
                df = df - 1
            End If
            Dim ratio As Decimal = (standardscore * Math.Sqrt(2 * df)) + df
            VarGroupMean = VarGroupMean * ratio / df
        End If

        Dim VarGamma11 As Decimal = VarGroupMean * J / (nT * nC)
    ...
    End Function
    Public Function FindVar(ByVal J As Integer, ByVal pT As Double, ByVal nIndiv As Integer, ByVal residualBetween As Double, ByVal residualWithin As Double, Optional ByVal degreeCertainty As Double = -1) As Double
        'Dim INV() As Decimal = MInverseV(residualBetween, residualWithin, nIndiv)
        'Dim f As Decimal = INV(1)
        ...
        Dim VarGamma11 As Decimal = VarGroupMean / (J * pT * (1 - pT))
    ...
    Public Function FindVarForWidth(ByVal Width As Double, ByVal WidthLevel As Double, Optional ByVal degreeCertainty As Double = -1, Optional ByVal J As Double = 10, Optional ByVal covariate As Boolean = False) As Double
        Dim dcCorrection As Double = 1
        If degreeCertainty <> -1 Then
            Dim standardscore As Decimal = InvNormalDistribution(degreeCertainty)
            Dim df As Decimal = J - 2
            If covariate Then
                df = df - 1
            End If
            Dim approxChi As Decimal = (standardscore * Math.Sqrt(2 * df)) + df
            dcCorrection = df / approxChi
        End If
        Dim SideEdge As Double = (1 - WidthLevel) / 2
        Dim CritValue As Double = InvNormalDistribution(1 - SideEdge)
        Return ((Width / (2 * CritValue)) ^ 2) * dcCorrection
    End Function
