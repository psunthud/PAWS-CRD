    ...
    Private Sub run()
        ...
        If (Proceed = True) Then
            ...
            ElseIf (TabControl1.SelectedIndex = 1) Then
                ...
                NIndivValue = StartValue(2)
                NTValue = StartValue(0)
                NCValue = StartValue(1)
                PTValue = StartValue(0) / (StartValue(0) + StartValue(1))
                NGValue = (StartValue(0) + StartValue(1))
                If MonteCarlo.Checked = True Then
                    ...
                    If Not (StartValue(0) = -1 Or StartValue(1) = -1 Or StartValue(2) = -1 Or pT = -1) Then
                        ...
                        NIndivValue = FinalValue(2)
                        NTValue = FinalValue(0)
                        NCValue = FinalValue(1)
                        PTValue = FinalValue(0) / (FinalValue(0) + FinalValue(1))
                        NGValue = (FinalValue(0) + FinalValue(1))
                    Else
                        NIndiv2.Text = "N/A"
                        NTGroups2.Text = "N/A"
                        NCGroups2.Text = "N/A"
    ...
    Private Sub OutputCalcDegree2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OutputCalcDegree2.Click
        If MonteCarlo.Checked = True Then
            If IsOutputDegree2.Checked = True Then
                If IsDoubleInRange(OutputDegreeLevel2, 0.001, 0.999, "Degree of Assurance") Then
        ...
        Else
            If IsOutputDegree2.Checked = True Then
                If IsDoubleInRange(OutputDegreeLevel2, 0.001, 0.999, "Degree of Assurance") Then
                    ...
                    Width99OutputLabel2C.ForeColor = Color.Blue
                    If checkCovariateInstanceVariable Then
                        Dim residual() As Double = FindResidual(True, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue, ICCZValue, R2ClusterZValue, R2IndivZValue)
                        Dim resultVar As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residual(0), residual(1), CDbl(OutputDegreeLevel2.Text))
                        ObtainedWidth95C.Text = PrintWidth(ESValue, FindWidthCIES(resultVar, 0.95)) 'Storage.Width(Position, 2).ToString("f3") & " (" & (ESValue - Storage.Width(Position, 2) / 2).ToString("f3") & ", " & (ESValue + Storage.Width(Position, 2) / 2).ToString("f3") & ")"
                        ObtainedWidth99C.Text = PrintWidth(ESValue, FindWidthCIES(resultVar, 0.99)) 'Storage.Width(Position, 3).ToString("f3") & " (" & (ESValue - Storage.Width(Position, 3) / 2).ToString("f3") & ", " & (ESValue + Storage.Width(Position, 3) / 2).ToString("f3") & ")"
                        If RunNullModelInstanceVariable Then
                            Dim residualNull() As Double = FindResidual(False, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue)
                            Dim resultVarNull As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residualNull(0), residualNull(1), CDbl(OutputDegreeLevel2.Text))
                            ObtainedWidth95.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.95)) 'Storage.Width(Position, 0).ToString("f3") & " (" & (ESValue - Storage.Width(Position, 0) / 2).ToString("f3") & ", " & (ESValue + Storage.Width(Position, 0) / 2).ToString("f3") & ")"
                            ObtainedWidth99.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.99)) 'Storage.Width(Position, 1).ToString("f3") & " (" & (ESValue - Storage.Width(Position, 1) / 2).ToString("f3") & ", " & (ESValue + Storage.Width(Position, 1) / 2).ToString("f3") & ")"
                        End If
                    Else
                        Dim residualNull() As Double = FindResidual(False, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue)
                        Dim resultVarNull As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residualNull(0), residualNull(1), CDbl(OutputDegreeLevel2.Text))
                        ObtainedWidth95.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.95)) 'Storage.Width(Position, 0).ToString("f3") & " (" & (ESValue - Storage.Width(Position, 0) / 2).ToString("f3") & ", " & (ESValue + Storage.Width(Position, 0) / 2).ToString("f3") & ")"
                        ObtainedWidth99.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.99)) 'Storage.Width(Position, 1).ToString("f3")
                    End If
                End If
            Else
                IsOutputDegree2.ForeColor = Color.Black
                ...
                If checkCovariateInstanceVariable Then
                    Dim residual() As Double = FindResidual(True, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue, ICCZValue, R2ClusterZValue, R2IndivZValue)
                    Dim resultVar As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residual(0), residual(1), -1)
                    ObtainedWidth95C.Text = PrintWidth(ESValue, FindWidthCIES(resultVar, 0.95)) 'Storage.Width(Position, 2).ToString("f3")
                    ObtainedWidth99C.Text = PrintWidth(ESValue, FindWidthCIES(resultVar, 0.99)) 'Storage.Width(Position, 3).ToString("f3")
                    If RunNullModelInstanceVariable Then
                        Dim residualNull() As Double = FindResidual(False, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue)
                        Dim resultVarNull As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residualNull(0), residualNull(1), -1)
                        ObtainedWidth95.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.95)) 'Storage.Width(Position, 0).ToString("f3")
                        ObtainedWidth99.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.99)) 'Storage.Width(Position, 1).ToString("f3")
                    End If
                Else
                    Dim residualNull() As Double = FindResidual(False, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue)
    ...
    Private Sub OutputCalcDegree_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OutputCalcDegree.Click
        If MonteCarlo.Checked = True Then
            If IsOutputDegree.Checked = True Then
                If IsDoubleInRange(OutputDegreeLevel, 0.001, 0.999, "Degree of Assurance") Then
        ...
        Else
            If IsOutputDegree.Checked = True Then
                If IsDoubleInRange(OutputDegreeLevel, 0.001, 0.999, "Degree of Assurance") Then
                    ...
                    If checkCovariateInstanceVariable Then
                        Dim residual() As Double = FindResidual(True, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue, ICCZValue, R2ClusterZValue, R2IndivZValue)
                        Dim resultVar As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residual(0), residual(1), CDbl(OutputDegreeLevel.Text))
                        WidthCI95C.Text = PrintWidth(ESValue, FindWidthCIES(resultVar, 0.95)) 'Storage.Width(Position, 2).ToString("f3")
                        WidthCI99C.Text = PrintWidth(ESValue, FindWidthCIES(resultVar, 0.99)) 'Storage.Width(Position, 3).ToString("f3")
                        If RunNullModelInstanceVariable Then
                            Dim residualNull() As Double = FindResidual(False, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue)
                            Dim resultVarNull As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residualNull(0), residualNull(1), CDbl(OutputDegreeLevel.Text))
                            WidthCI95.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.95)) 'Storage.Width(Position, 0).ToString("f3")
                            WidthCI99.Text = PrintWidth(ESValue, FindWidthCIES(resultVarNull, 0.99)) 'Storage.Width(Position, 1).ToString("f3")
                        End If
                    Else
                        Dim residualNull() As Double = FindResidual(False, CDbl(NTValue) / NGValue, NGValue, NIndivValue, ESValue, ICCYValue)
                        Dim resultVarNull As Decimal = Storage.FindVar(NTValue, NCValue, NIndivValue, residualNull(0), residualNull(1), CDbl(OutputDegreeLevel.Text))
